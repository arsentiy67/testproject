...
public class CustomPageFactory {
  ...
  private static void initElement(WebDriver driver, Field field, Object section) {
    try {
      ...
      if (instance != null) {
        ...
        if (isClass(type, CustomWebElement.class) || isClass(type, WebElementsList.class)) {
          CustomWebElement el = (CustomWebElement) instance;
  ...
  private static Object newInstance(WebDriver driver, Field field) {
    if (isClass(field, WebElementsList.class) || isList(field)) {
      return new WebElementsList(driver, field.getName());
    }
    if (isInterface(field, WebElement.class) || isInterface(field, CustomWebElement.class)) {
      return new CustomWebElement(driver, field.getName());
    } else {
      return null;
    }
  }
